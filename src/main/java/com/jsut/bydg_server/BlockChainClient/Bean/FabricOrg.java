...
public class FabricOrg {
    ...
    public FabricOrg(Peers peers, Orderers orderers, FabricStore fabricStore, String cryptoConfigPath)
        ...
        //InputStream is=this.getClass().getResourceAsStream(cryptoConfigPath+"/peerOrganizations/"+peers.getOrgDomainName()+String.format("/users/Admin@%s/msp/keystore", peers.getOrgDomainName()));



//        InputStream inputStream=this.getClass().getResourceAsStream("/fabric/crypto-config/peerOrganizations/org1.imocc.com/users/Admin@org1.imocc.com/msp/keystore");
//        byte[] byt = new byte[inputStream.available()];
//        inputStream.read(byt);
//        File skFile=new File();
//        if (!skFile.exists()){
//            skFile.createNewFile();
//        }
//        OutputStream outputStream=new FileOutputStream(skFile);
//        BufferedOutputStream bufferedOutput = new BufferedOutputStream(outputStream);
//        bufferedOutput.write(byt);
//        System.out.println("资源是否成功获取："+skFile.exists());
...
//        File skFile=new File(cryptoConfigPath+"/peerOrganizations/"+peers.getOrgDomainName()+String.format("/users/Admin@%s/msp/keystore", peers.getOrgDomainName()));
//        if (!skFile .getParentFile().exists()) {
//            skFile.getParentFile().mkdirs();
//        }
//        if (!skFile.exists()){
//            skFile.createNewFile();
//        }
...
//        File certificateFile=new File(cryptoConfigPath+"/peerOrganizations/"+peers.getOrgDomainName()+String.format("/users/Admin@%s/msp/signcerts/Admin@%s-cert.pem", peers.getOrgDomainName(), peers.getOrgDomainName()));
//        System.out.println(cryptoConfigPath+"/peerOrganizations/"+peers.getOrgDomainName()+String.format("/users/Admin@%s/msp/signcerts/Admin@%s-cert.pem", peers.getOrgDomainName(), peers.getOrgDomainName()));
//        if (!certificateFile .getParentFile().exists()) {
...
//        }
        ...
        System.out.println("skFile="+"存在"+skFile+skFile.exists());
        System.out.println("certificationFile="+"存在"+certificateFile+certificateFile.exists());
        log.debug("certificateFile = " + certificateFile.getAbsolutePath());
        System.out.println("certificateFile="+cryptoConfigPath);
    ...
    private File findFileSk(File directory) {
        ...
        System.out.println("文件的数量"+matches.length);
        for (int i = 0; i < matches.length; i++) {
            System.out.println(matches[i]);
            if (!matches[i].getName().endsWith("_sk")){
                matches[i]=null;
            }
